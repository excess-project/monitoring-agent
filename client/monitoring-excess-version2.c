	...
	while (i < 4) {
		...
		if (strchr(ptr, 'k') != NULL ) {
			if (ptr != NULL ) {
				switch (i) {
				case (0):
					infos_mem->memory_total = atoi(ptr);
					break;
				case (1):
					infos_mem->memory_used = atoi(ptr);
					break;
				case (2):
					infos_mem->memory_free = atoi(ptr);
					break;
				case (3):
					infos_mem->memory_buffers = atoi(ptr);
					break;
				default:
					;
				}
	...
	while (i < 4) {
		...
		if (strchr(ptr, 'k') != NULL ) {
			if (ptr != NULL ) {
				switch (i) {
				case (0):
					infos_mem->swap_total = atoi(ptr);
					break;
				case (1):
					infos_mem->swap_used = atoi(ptr);
					break;
				case (2):
					infos_mem->swap_free = atoi(ptr);
					break;
				case (3):
					infos_mem->swap_cached = atoi(ptr);
					break;
				default:
					;
				}
	...
	while (i < 5) {
		...
		if (strchr(ptr, '.') != NULL ) {
			if (ptr != NULL ) {
				switch (i) {
				case (0):
					infos_cpu->cpu_user = strtod(ptr, NULL );
					break;
				case (1):
					infos_cpu->cpu_sys = strtod(ptr, NULL );
					break;
				case (2):
					infos_cpu->cpu_nice = strtod(ptr, NULL );
					break;
				case (3):
					infos_cpu->cpu_idle = strtod(ptr, NULL );
					break;
				case (4):
					infos_cpu->cpu_wa_io = strtod(ptr, NULL );
					break;
				default:
					;
				}
...
int excess_sensor(sensor_msg_t * info) {
	...
	while (CURRENT_INDEX >= 0) {
		...
		to_send_msg[CURRENT_INDEX].cpu_used = infocpu.cpu_user + infocpu.cpu_sys
				+ infocpu.cpu_nice; //load
		...
		to_send_msg[CURRENT_INDEX].ram_used = (infomem.memory_used
				+ (infomem.memory_buffers + infomem.swap_cached)) * 100
				/ (infomem.memory_total + infomem.swap_total);
		to_send_msg[CURRENT_INDEX].ram_avail = (infomem.memory_used
				- (infomem.memory_buffers + infomem.swap_cached)) * 100
				/ (infomem.memory_total + infomem.swap_total);
